...
Two main software design patterns that were considered for this solution were Flux and MVC, from the start the decision was made to use React.js as a view - (the part of the application that shows current data state), while researching React.js the preffered way to write the app for many programmers seemed to be Flux, although there was no clear advandage of Flux over MVC. MVC stands for Model View Controller, the first part, the model represents the logic, and data that is being displayed, the view is the part that displays the information, and controller accepts the user input and sends it either to model or view, these are the main parts of the application that communicate with eachother. One can be very creative while implementing MVC, and when unexperienced programmer tries it, it can become very difficult to manage all the different parts and especcialy complicated when having a lot of different components that communicate with eachother. Even though Autograder is not as huge and complicated as some sites its still not a small application, there are many elements that depend on data from different places. One example would be the main part of the Autograder which is the lab overview where teachers can have an easy access to all the assignments and see individual results. 
\begin{figure}[h]
...
\end{figure}
Designing it with MVC would look as follows:

\begin{tikzpicture}[node distance = 2cm, auto]
 \node [block] (model) {Model};
 \node [block, right of=model] (view) {View};
 \node [block, right of=view] (controller) {Controller};
 \path [line] (model) -- (identify);
 \path [line] (view) -- (identify);
 \path [line] (controller) -- (identify);
\end{tikzapicture}
